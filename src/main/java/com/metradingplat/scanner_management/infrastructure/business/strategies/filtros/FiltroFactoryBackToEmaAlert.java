...
public class FiltroFactoryBackToEmaAlert implements IFiltroFactory {
    ...
    public Filtro obtenerFiltro(Map<EnumParametro, Valor> valoresSeleccionados) {
        ...
        parametros.add(this.crearParametroPeriodoEma(
                (ValorInteger) valoresSeleccionados.get(EnumParametro.PERIODO_EMA_BACK_TO_EMA)));
        parametros.add(this
                .crearParametroTimeframe((ValorString) valoresSeleccionados.get(EnumParametro.TIMEFRAME_BACK_TO_EMA)));
        // Sin esto el filtro pasaba incondicionalmente con cualquier simbolo
        // con >= 3 velas, sin importar si de verdad se estaba acercando a la
        // EMA (compute_value devuelve 0.0 si no, o el % con signo si si --
        // FUERA de una banda casi-cero captura "si se acerco".
    ...
    private Parametro crearParametroTimeframe(ValorString valorUsuario) {
        ...
        ValorString valor = new ValorString(
                ...
                enumTipoValor,
                enumValor.name());
        return new Parametro(EnumParametro.TIMEFRAME_BACK_TO_EMA, EnumParametro.TIMEFRAME_BACK_TO_EMA.getEtiqueta(),
                valor, opciones);
    }
    ...
    public List<ResultadoValidacion> validarValoresSeleccionados(Map<EnumParametro, Valor> valoresSeleccionados) {
        ...
        this.objValidador.validarStringConOpciones(this.enumFiltro, EnumParametro.TIMEFRAME_BACK_TO_EMA,
                valoresSeleccionados.get(EnumParametro.TIMEFRAME_BACK_TO_EMA), TIMEFRAMES_SOPORTADOS)
                .ifPresent(errores::add);
