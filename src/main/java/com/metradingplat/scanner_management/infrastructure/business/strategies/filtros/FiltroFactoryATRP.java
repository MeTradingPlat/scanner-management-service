...
public class FiltroFactoryATRP implements IFiltroFactory {
        ...
        public Filtro obtenerFiltro(Map<EnumParametro, Valor> valoresSeleccionados) {
                ...
                parametros.add(
                                this.crearParametroTimeframe(
                                                (ValorString) valoresSeleccionados.get(EnumParametro.TIMEFRAME_ATRP)));
                parametros.add(
                                this.crearParametroPeriodoAtr((ValorInteger) valoresSeleccionados
        ...
        private Parametro crearParametroTimeframe(ValorString valorUsuario) {
                ...
                ValorString valor = new ValorString(
                                ...
                                enumTipoValor,
                                enumValor.name());
                return new Parametro(EnumParametro.TIMEFRAME_ATRP, EnumParametro.TIMEFRAME_ATRP.getEtiqueta(), valor,
                                opciones);
        }

        private Parametro crearParametroPeriodoAtr(ValorInteger valorUsuario) {
                EnumTipoValor enumTipoValor = EnumTipoValor.INTEGER;
                List<Valor> opciones = this.obtenerOpciones(new IEnumValores[0]); // No hay opciones de enum para
                                                                                  // periodo
                ValorInteger valor = new ValorInteger(
                                "etiqueta.vacia",
                                enumTipoValor,
                                valorUsuario != null ? valorUsuario.getValor() : 14);
                return new Parametro(EnumParametro.PERIODO_ATR_ATRP, EnumParametro.PERIODO_ATR_ATRP.getEtiqueta(),
                                valor,
                                opciones);
        }
        ...
        private Parametro crearParametroTipoPromedioMovil(ValorString valorUsuario) {
                ...
                ValorString valor = new ValorString(
                                ...
                                enumTipoValor,
                                enumValor.name());
                return new Parametro(EnumParametro.TIPO_PROMEDIO_MOVIL_ATRP,
        ...
        public List<ResultadoValidacion> validarValoresSeleccionados(Map<EnumParametro, Valor> valoresSeleccionados) {
                ...
                this.objValidador
                                .validarCondicional(this.enumFiltro, EnumParametro.CONDICION,
                                                valoresSeleccionados.get(EnumParametro.CONDICION), 0.1f, 50.0f)
                                .ifPresent(errores::add);

                this.objValidador.validarStringConOpciones(this.enumFiltro, EnumParametro.TIMEFRAME_ATRP,
                                valoresSeleccionados.get(EnumParametro.TIMEFRAME_ATRP), TIMEFRAMES_SOPORTADOS)
                                .ifPresent(errores::add);

                this.objValidador
                                .validarInteger(this.enumFiltro, EnumParametro.PERIODO_ATR_ATRP,
                                                valoresSeleccionados.get(EnumParametro.PERIODO_ATR_ATRP),
                                                5, 100)
                                .ifPresent(errores::add);
